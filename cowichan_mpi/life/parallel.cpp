...
life_mpi(
  ...
  for (i=0; (i<iters) && alive; i++){
    ...
    life_row_mpi(world, count, nr, nc, 0, nr-1, 1);
    for (r=1; r<(nr-1); r++){
      life_row_mpi(world, count, nr, nc, r, r-1, r+1);
    }
    life_row_mpi(world, count, nr, nc, nr-1, nr-2, 0);
    /* update cells */
    alive = 0;
    for (r=0; r<nr; r++){
      for (c=0; c<nc; c++){
...
life_row_mpi(
  ...
  life_one_mpi(world, count, r, r_lo, r_hi, 0, nc-1, 1);
  for (c=1; c<(nc-1); c++){
    life_one_mpi(world, count, r, r_lo, r_hi, c, c-1, c+1);
  }
  life_one_mpi(world, count, r, r_lo, r_hi, nc-1, nc-2, 0);
